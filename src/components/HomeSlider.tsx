    ...
    <div className={cn("relative h-screen w-full overflow-hidden", className)}>
      ...
      {slides.map((slide, index) => (
        ...
        >
          ...
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center px-4 text-center">
            <h1 
              className={cn(
                "text-4xl md:text-6xl lg:text-7xl font-playfair font-bold mb-4 md:mb-6 tracking-wide max-w-4xl",
            ...
            {slide.subtitle && (
              <p 
                className={cn(
                  "text-lg md:text-xl mb-8 max-w-2xl font-light",
                  index === activeIndex ? 'animate-fadeIn' : 'opacity-0'  
                )}
      ...
      <div className="absolute left-0 right-0 top-1/2 -translate-y-1/2 flex justify-between px-4 md:px-8 z-20">
        <Button 
          variant="ghost" 
          size="icon" 
          className="bg-dark/20 hover:bg-dark/50 text-white rounded-full transition-all"
          onClick={prevSlide}
        >
        ...
        {slides.map((_, index) => (
          <button
            ...
            className={cn(
              ...
              index === activeIndex ? "bg-gold w-8" : "bg-white/50 hover:bg-white/80"
            )}
